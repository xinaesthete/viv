...
import { DrawPolygonByDraggingMode, type DraggingEvent, type FeatureCollection, type ModeProps, type StartDraggingEvent, type StopDraggingEvent } from "@deck.gl-community/editable-layers";
...
export class DrawRectangleByDraggingMode extends DrawPolygonByDraggingMode {
  ...
  handleDraggingAux(event: DraggingEvent, props: ModeProps<FeatureCollection>) {
    ...
    if (features.length > 0 && features[features.length - 1].properties?.tentativeRectangle) {
      // biome-ignore lint/suspicious/noExplicitAny: deck-gl-community/editable-layers types are a bit bad?
      features[features.length - 1] = feature as any;
    } else {
      // biome-ignore lint/suspicious/noExplicitAny: deck-gl-community/editable-layers types are a bit bad?
